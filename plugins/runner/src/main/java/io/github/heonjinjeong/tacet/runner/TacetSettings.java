...
/**
 * Options of one `tacet ci` run, shared by the Gradle and Maven plugins.
 *
...
public record TacetSettings(
    ...
    public TacetSettings {
        if (frontendDir == null) throw new IllegalArgumentException("Tacet: `frontendDir` is required");
        if (backendDir == null) throw new IllegalArgumentException("Tacet: `backendDir` is required");
    ...
    private static void require(String name, String value, Set<String> allowed) {
        if (!allowed.contains(value)) {
            throw new IllegalArgumentException("Tacet: `" + name + "` must be one of " + allowed + " but was '" + value + "'");
        }
    }
}
