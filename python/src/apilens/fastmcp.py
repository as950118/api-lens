"""Register ApiLens tools on a (Python) FastMCP server.
...
from .client import ApiLens

GraphMode = Literal["none", "mermaid", "json"]


def register_tools(mcp: Any, lens: ApiLens | None = None, *, prefix: str = "", **lens_options: Any) -> list[str]:
    """
    Adds the ApiLens tools to `mcp` (anything with FastMCP's `tool(fn, name=..., ...)`).

    Pass an existing `ApiLens`, or its constructor options (index, frontend_dir, backend_dir, config, command, ...).
    Returns the registered tool names.
    """
    lens = lens or ApiLens(**lens_options)

    def index_frontend(files: list[str] | None = None, changed_since: str | None = None) -> dict[str, Any]:
        """Analyze the TypeScript frontend and update the ApiLens index. Pass `files` (or `changed_since`, a git ref)
